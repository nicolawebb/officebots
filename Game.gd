...
#var SERVER_URL= "ws://127.0.0.1:6969"
...
remote func create_file(name): 
    ...
    file.store_line( "ID, fps, time, pos_x,pos_y,pos_z,r_x,r_y,r_z, expression, is_speaking, mission 1, mission 2, mission 3, score ")
...
func pre_save(): 
    for p in $Players.get_children(): 
        ...
        var data = "%s"%ID + "," + "%s"%fps + "," + "%s"%time+ "," + "%.2f"%p.global_transform.origin[0]+ "," + "%.2f"%p.global_transform.origin[1]+"," + "%.2f"%p.global_transform.origin[2] +"," +"%.2f"%p.rotation_degrees[0]+"," + "%.2f"%p.rotation_degrees[1]+ "," + "%.2f"%p.rotation_degrees[2] + "," + mood + "," + "%s"%p.is_speaking() + "," + "%s"%p.mission_1 + "," + "%s"%p.mission_2 + "," + "%s"%p.mission_3 + "," + "%s"%p.score
        save_data(ID,data)
...
remote func save_q5(who,prolific_id,results): 
    ...
    var path_modified = path + "/%s"%who + "big5" + ".csv"
